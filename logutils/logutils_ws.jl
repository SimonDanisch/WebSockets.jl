...
export WebSocketLogger, shouldlog
export main # temp
...
catch_exceptions(logger::WebSocketLogger) = true
"Handle a log event"
function handle_message(logger::WebSocketLogger, level, message, _module, group, id,
    ...
    ##### TODO message can be a tuple here. Modify the old dispatch mechanism.
    strbody = string(typeof(message)) * " " * string(message) *"\n"
...
function main()
    ...
    #@logmsg 1 message [key=value | value ...]
    #retu = nothing
...
"""
Specialized show methods for this logger.
Somewhat like 'write', return the number of
printed characters.
"""
_show
"Fallback for unspecified types"
function _show(io::IOBuffer, arg)
    l0 = io.size
    show(io, arg)
    io.size - l0
end
function _show(io::IOContext, arg)
    l0 = io.io.size
    show(io, arg)
    io.io.size - l0
end

"Switch color for following output, don't add to returned length"
function _show(io::IO, sy::Symbol)
    ...
    if co != ""
        ...
        # This takes no space on the output line
        return 0
    else
        # The symbol is not a color. Return the number of printed characters.
        write(io, ":")
        return write(io, sy) + 1
    end
end
